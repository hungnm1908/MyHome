...
-(void)selectToday
{
    ...
    NSInteger monthCount = self.months.count;
    int index = (int)(self.todayIndexPath.row % monthCount) + 1;
    monthNameStr = [NSString stringWithFormat:@"%@%d",index<10?@"0":@"",index];
    
    NSInteger yearCount = self.years.count;
    yearNameStr = [self.years objectAtIndex:(self.todayIndexPath.section % yearCount)];
}
...
- (void)pickerView:(UIPickerView *)pickerView didSelectRow:(NSInteger)row inComponent:(NSInteger)component {
    if(component == MONTH) {
        NSInteger monthCount = self.months.count;
        int index = (int)(row % monthCount) + 1;
        monthNameStr = [NSString stringWithFormat:@"%@%d",index<10?@"0":@"",index];
    }else{
        NSInteger yearCount = self.years.count;
        yearNameStr = [self.years objectAtIndex:(row % yearCount)];
    }
    
    [self.monthDelegate didSelectCell:[NSString stringWithFormat:@"%@/%@",monthNameStr,yearNameStr]];
}
